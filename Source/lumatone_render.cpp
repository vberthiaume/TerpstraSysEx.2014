...
{
    auto numBoards = TerpstraSysExApplication::getApp().getNumBoards();
    auto octaveSize = TerpstraSysExApplication::getApp().getOctaveBoardSize();
    lumatoneGeometry = LumatoneGeometry(GetLumatoneBoardSize(octaveSize));
    ...
    keyCentres = tilingGeometry.getHexagonCentresSkewed(lumatoneGeometry, 0, numBoards);
}

juce::Array<juce::Point<float>> LumatoneRender::getKeyCentres()
{
    return tilingGeometry.getHexagonCentresSkewed(lumatoneGeometry, 0, TerpstraSysExApplication::getApp().getNumBoards());
}
...
void LumatoneRender::render(LumatoneAssets::LumatoneGraphicRenderSize maxRenderSize)
{
    ...
    auto numBoards = TerpstraSysExApplication::getApp().getNumBoards();
    auto octaveSize = TerpstraSysExApplication::getApp().getOctaveBoardSize();
    LumatoneColourModel* colourModel = TerpstraSysExApplication::getApp().getColourModel();
    ...
    for (int boardIndex = 0; boardIndex < numBoards; boardIndex++)
    {
        ...
        {
            juce::Colour keyColour = TerpstraSysExApplication::getApp().getMappingData()->readKey(boardIndex, keyIndex)->colour;
