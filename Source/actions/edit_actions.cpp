...
bool SingleNoteAssignAction::perform()
{
    if (boardId > 0 && boardId <= numOctaveBoards && keyIndex >= 0 && keyIndex < octaveBoardSize)
    {
        ...
        if (setKeyType || setChannel || setNote || setColour || setCCFaderPolarity)
        {
            ...
            if (onlyColor)
            {
                controller->sendKeyColourConfig(boardId, keyIndex, newData.colour);
            }
            else
            {
                // Send to device
...
bool SectionEditAction::perform()
{
    ...
    {
        // Send to device
        controller->sendAllParamsOfBoard(boardId, newData);
...
bool SectionEditAction::undo()
{
    ...
    {
        // Send to device
        controller->sendAllParamsOfBoard(boardId, previousData);
...
void MultiKeyAssignAction::applyMappedKeyData(const juce::Array<MappedLumatoneKey>& newKeys, const juce::Array<MappedLumatoneKey>& oldKeys)
{
    ...
    {
        MappedLumatoneKey oldKey = oldKeys[i];
        MappedLumatoneKey newKey = newKeys[i];

        if (!oldKey.configIsEqual(newKey))
        {
            controller->sendKeyConfig(newKey.boardIndex + 1, newKey.keyIndex, newKey.noteNumber, newKey.channelNumber, newKey.keyType, newKey.ccFaderDefault);
        }

        if (!oldKey.colourIsEqual(newKey))
        {
            controller->sendKeyColourConfig(newKey.boardIndex + 1, newKey.keyIndex, newKey.colour);
        }
    }
}
