...
LumatoneKeyUpdateBuffer::LumatoneKeyUpdateBuffer(TerpstraMidiDriver& driverIn, LumatoneController* stateIn)
    ...
    , state(stateIn)
{
    ...
    auto mapping = state->getMappingData();
    for (int b = 0; b < MAX_LUMATONE_BOARDS; b++)
    {
        if (b < state->getNumBoards())
        {
            const LumatoneBoard& board = mapping.getBoard(b);
...
{
    return boardIndex * state->getOctaveBoardSize() + keyIndex;
}

LumatoneKeyCoord LumatoneKeyUpdateBuffer::getKeyCoord(int keyNum) const
{
    return LumatoneKeyCoord(keyNum / state->getOctaveBoardSize(), keyNum % state->getOctaveBoardSize());
}
...
void LumatoneKeyUpdateBuffer::updateKeyConfig(int boardIndex, int keyIndex, const LumatoneKey& config)
{
    ...
    const LumatoneKey& currentKey = state->getKey(boardIndex, keyIndex);
    ...
    for (int board = 0; board < MAX_LUMATONE_BOARDS; board++)
    {
        ...
        for (int keyIndex = 0; keyIndex < MAX_LUMATONE_BOARD_KEYS; keyIndex++)
        {
            ...
            if (!lastUpdatedKey.colourIsEqual(keyUpdate))
            {
                ...
                if (state->getConfirmedVersion() >= LumatoneFirmware::ReleaseVersion::VERSION_1_0_11)
