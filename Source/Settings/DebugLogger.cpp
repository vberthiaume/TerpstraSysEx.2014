...
void LumatoneSandboxLogger::Log(DebugLogEntry info)
{
...
void LumatoneSandboxLogger::log(LogStatus status, LogType type, juce::String method, juce::String message) const
{
    ...
    DebugLogEntry info = createLog(status, type, method, message);
...
DebugLogEntry LumatoneSandboxLogger::createLog(LogStatus status, LogType type, juce::String method, juce::String message) const
{
...
juce::String DebugLogEntry::toFullString() const
{
...
juce::String DebugLogEntry::toShortString() const
{
    juce::StringArray tokens;
...
juce::String DebugLogEntry::getStatusString() const
{
    switch (status)
    {
    default:
...
juce::String DebugLogEntry::getTypeString() const
{
    switch (type)
    {
        default:
...
DebugLogEntry DebugLogEntry::FromString(juce::String logString)
{
    DebugLogEntry logInfo;
    ...
    for (int i = 1; i < logString.length(); i++)
    {
        if (logString[i] == delim || i == lastIndex)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                logInfo.status = DebugLogEntry::LogStringToStatus(word);
                break;
            case 3:
                logInfo.type = DebugLogEntry::StringToLogType(word);
                break;
            case 4:
                logInfo.method = word;
                break;
...
LogStatus DebugLogEntry::CodeToStatus(int statusCode)
{
    if (statusCode == 0)
...
LogStatus DebugLogEntry::LogStringToStatus(juce::StringRef statusString)
{
...
LumatoneEditorLogger::LogType DebugLogEntry::StringToLogType(juce::StringRef typeString)
{
    if (typeString == juce::String("Device"))
        return LogType::DEVICE;
    if (typeString == juce::String("MIDI"))
        return LogType::MIDI;
    if (typeString == juce::String("Sysex"))
        return LogType::SYSEX;
